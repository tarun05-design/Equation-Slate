...
# Register non-meta buffer on TrOCRPositionalEmbedding modules
for m in model.modules():
    if hasattr(m, "_float_tensor"):
        m.register_buffer("_float_tensor", torch.tensor([0.0], dtype=torch.float32), persistent=False)

print("Non-meta buffer registered!")

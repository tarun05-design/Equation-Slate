...
# Target TrOCRPositionalEmbedding modules specifically
for m in model.modules():
    if "TrOCRPositionalEmbedding" in type(m).__name__:
        print("Found TrOCRPositionalEmbedding:", type(m))
        m._float_tensor = torch.tensor([0.0], dtype=torch.float32, device="cpu")
        m.register_buffer("_float_tensor", torch.tensor([0.0], dtype=torch.float32, device="cpu"), persistent=False)
